...
def create_criteo_dataset(file_path, embed_dim=8, test_size=0.2):
    ...
    # onehot_data的列为：C1_0-C26-XXX , (数据总数×36224)
    # get_dummies函数是利用pandas实现one hot encode的方式；


    # 准备训练输入 X 和标签 y  df=df.loc[:,[‘name1’,‘name2’,‘name3’]]
    X = data.drop('label',axis=1) # X的列为：I1-I13 C1-C26 , (数据总数×39)
    y = data[['label']]


    # 特征拼接
    X = pd.concat([X, onehot_data], axis=1) # X的列为：I1-I13 C1-C26 C1_0-C26-XXX , (数据总数×36263)
